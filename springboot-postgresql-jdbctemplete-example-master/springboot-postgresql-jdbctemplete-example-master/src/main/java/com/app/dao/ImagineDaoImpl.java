...
import com.app.rowmapper.ImagineRowMapper;
...
@Repository
public class ImagineDaoImpl implements ImagineDao {
	...
	private static final Logger logger = LogManager.getLogger(ImagineDaoImpl.class);
		...
		public void addEmployee(Employee employee) {
				...
				if(employee!=null) {
					String addEmployee_sql = "INSERT INTO imagine (name,define, times,addtime,"
		...
		public List<Employee> getEmployees()  {
			...
			String sql="select * from imagine";
			logger.info(sql);
	...
	public List<Employee> findEmployeeByName(String name)  {
		...
		String sql="select * from imagine where name ='"+name+"'";
		logger.info(sql);
		List<Employee> employee=template.query(sql, new EmployeeRowMapper());
		return employee;
	}
		@Override
		 public Employee findEmployeeById(int id) {
		  String query = "SELECT * FROM imagine WHERE id = ?";
		...
		public boolean existsById(Integer id) {
		    return jdbcTemplate.queryForObject(
		        "SELECT EXISTS(SELECT 1 FROM imagine where id=?)", Boolean.class, id);
		}


		public void updateEmployee(Employee employee){
			String sql="update imagine set name=:name,"
		...
		public void deleteEmployee(Integer id) {
			...
			String sql="DELETE FROM imagine WHERE id= ?";
