...
import com.app.rowmapper.LogicMapper;
...
public class LogicDaoImpl implements LogicDao {
	...
	private static final Logger logger = LogManager.getLogger(LogicDaoImpl.class);
		...
		public void addEmployee(Employee employee) {
				...
				if(employee!=null) {
					String addEmployee_sql = "INSERT INTO logic (name,define, times,addtime,"
	...
	public List<Employee> findEmployeeByName(String name)  {
		...
		String sql="select * from logic where name ='"+name+"'";
		logger.info(sql);
		List<Employee> employee=template.query(sql, new EmployeeRowMapper());
		return employee;
	}
		...
		@Override
		 public Employee findEmployeeById(int id) {
		  String query = "SELECT * FROM logic WHERE id = ?";
		...
		public boolean existsById(Integer id) {
		    return jdbcTemplate.queryForObject(
		        "SELECT EXISTS(SELECT 1 FROM logic where id=?)", Boolean.class, id);
		}
		...
		public void updateEmployee(Employee employee){
			String sql="update logic set name=:name,"
		...
		public void deleteEmployee(Integer id) {
			String sql="DELETE FROM logic WHERE id= ?";
