...
import com.app.rowmapper.EqualRowMapper;
...
@Repository
public class EqualDaoImpl implements EqualDao {
	...
	private static final Logger logger = LogManager.getLogger(EqualDaoImpl.class);
		...
		public void addEmployee(Employee employee) {
				...
				if(employee!=null) {
					String addEmployee_sql = "INSERT INTO equal (name,define, times,addtime,"
	...
	public List<Employee> findEmployeeByName(String name)  {
		...
		String sql="select * from equal where name ='"+name+"'";
		logger.info(sql);
		List<Employee> employee=template.query(sql, new EmployeeRowMapper());
		return employee;
	}
		...
		@Override
		 public Employee findEmployeeById(int id) {
		  String query = "SELECT * FROM equal WHERE id = ?";
		...
		public boolean existsById(Integer id) {
		    return jdbcTemplate.queryForObject(
		        "SELECT EXISTS(SELECT 1 FROM equal where id=?)", Boolean.class, id);
		}


		public void updateEmployee(Employee employee){
			String sql="update equal set name=:name,"
		...
		public void deleteEmployee(Integer id) {
			...
			String sql="DELETE FROM equal WHERE id= ?";
