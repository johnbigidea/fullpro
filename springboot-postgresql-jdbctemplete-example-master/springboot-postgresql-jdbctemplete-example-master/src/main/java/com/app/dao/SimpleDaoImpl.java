...
public class SimpleDaoImpl implements SimpleDao {
	...
	private static final Logger logger = LogManager.getLogger(SimpleDaoImpl.class);
		...
		public void addEmployee(Employee employee) {
				...
				if(employee!=null) {
					String addEmployee_sql = "INSERT INTO simplerelation (name,define, times,addtime,"
		...
		public List<Employee> getEmployees()  {
			...
			String sql="select * from simplerelation";
			logger.info(sql);
	...
	public List<Employee> findEmployeeByName(String name)  {
		...
		String sql="select * from simplerelation where name ='"+name+"'";
		logger.info(sql);
		List<Employee> employee=template.query(sql, new EmployeeRowMapper());
		return employee;
	}
		@Override
		 public Employee findEmployeeById(int id) {
		  String query = "SELECT * FROM simplerelation WHERE id = ?";
		...
		public boolean existsById(Integer id) {
		    return jdbcTemplate.queryForObject(
		        "SELECT EXISTS(SELECT 1 FROM simplerelation where id=?)", Boolean.class, id);
		}


		public void updateEmployee(Employee employee){
			String sql="update simple set name=:name,"
		...
		public void deleteEmployee(Integer id) {
			...
			String sql="DELETE FROM simplerelation WHERE id= ?";
