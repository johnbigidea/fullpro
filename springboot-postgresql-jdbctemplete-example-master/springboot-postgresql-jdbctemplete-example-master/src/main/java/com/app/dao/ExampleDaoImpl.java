...
import com.app.rowmapper.ExampleRowMapper;
...
public class ExampleDaoImpl implements ExampleDao {
	...
	private static final Logger logger = LogManager.getLogger(ExampleDaoImpl.class);
		...
		public void addEmployee(Employee employee) {
				...
				if(employee!=null) {
					String addEmployee_sql = "INSERT INTO example (name,define, times,addtime,"
	...
	public List<Employee> findEmployeeByName(String name)  {
		...
		String sql="select * from example where name ='"+name+"'";
		logger.info(sql);
		List<Employee> employee=template.query(sql, new EmployeeRowMapper());
		return employee;
	}
		...
		public boolean existsById(Integer id) {
		    return jdbcTemplate.queryForObject(
		        "SELECT EXISTS(SELECT 1 FROM example where id=?)", Boolean.class, id);
		}


		public void updateEmployee(Employee employee){
			String sql="update example set name=:name,"
